...
class ImageManager {
	...
	class func imageFromFile(filePath: String) -> ResultOf<UIImage> {
		...
		if data == nil {
			...
			return returnError("ImageManager.imageFromFile(_)"
				,              "NSData(contentsOfFile:options:error)")

		}

		let img = UIImage(data:data!)
		let txt = "Unable to create the image from the data in the stored file."
		let userInfo = [NSLocalizedDescriptionKey: txt]
		if img == nil {
			return .Error(NSError(domain: ErrorDomain.PhotoStore.rawValue
				,                   code: ErrorCode.CouldntLoadImageProperties.rawValue
				,               userInfo: userInfo))
		}
	...
	class func makeStereogramWithLeftPhoto(leftPhoto: UIImage
		,                                  rightPhoto: UIImage) -> ResultOf<UIImage> {
		assert(leftPhoto.scale == rightPhoto.scale
			, "Image scales \(leftPhoto.scale) and \(rightPhoto.scale) must be the same.")
		let stereogramSize = CGSizeMake(leftPhoto.size.width + rightPhoto.size.width
			,                           max(leftPhoto.size.height, rightPhoto.size.height))
		...
		return .Error(NSError(domain: ErrorDomain.PhotoStore.rawValue
			,                   code: ErrorCode.CouldntCreateStereogram.rawValue
			,               userInfo: nil))
	}
