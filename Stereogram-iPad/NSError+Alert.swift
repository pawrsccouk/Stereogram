...
extension NSError {
    ...
    convenience init(errorCode code: ErrorCode
		,                  userInfo: [String: AnyObject]?) {
        self.init(domain: ErrorDomain.PhotoStore.rawValue
			,       code: code.rawValue
			,   userInfo: userInfo)
    }


    /// Class function to create an error to return when something failed but didn't say why.
    ///
    /// :param: location A string included in the error text and returned in the userInfo dict.
    ///         This should give a clue as to what operation was being performed.
    /// :param: target   A string included in the error text and returned in the userInfo dict.
    ///         This should give a clue as to which function returned this error
	///         (i.e. the function in 3rd party code that returned the unexpected error.)
    ...
    /// :param: caller The method or function which called the method that went wrong.
	///                I.e. where in my code it is.

    class func unknownErrorWithTarget(target: AnyObject
		,                             method: String
		,                             caller: String) -> NSError {
