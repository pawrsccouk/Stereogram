...
class ThumbnailCache {
    ...
    init() {
        ...
        notificationCenter.addObserverForName(UIApplicationDidReceiveMemoryWarningNotification, object: nil, queue: NSOperationQueue.mainQueue()) { (note: NSNotification!) -> Void in
    ...
    func thumbnailForImage(imagePath: FileName) -> ResultOf<UIImage> {
        ...
        return ImageManager.imageFromFile(imagePath).map { (img) -> ResultOf<UIImage> in
            self.addThumbnailForImage(img, atPath: imagePath)
        }
    }
    ...
    func addThumbnailForImage(image: UIImage, atPath path: FileName) -> ResultOf<UIImage> {
        // Use the left half of the image for the thumbnail, as having both makes the actual image content too small to see.
        return ImageManager.getHalfOfImage(image, whichHalf: .LeftHalf).map { leftHalf in
            let thumbnail = leftHalf.thumbnailImage(thumbnailSize: thumbSize, transparentBorderSize: 0, cornerRadius: 0, interpolationQuality: kCGInterpolationLow)
            self.thumbnailDict[path] = thumbnail
