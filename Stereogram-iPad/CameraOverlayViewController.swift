...
///
/// The view shows some help text and a crosshair.
/// It also uses it's own 'take photo' and 'cancel' buttons,
/// replacing the ones the camera would normally display.
...
class CameraOverlayViewController: UIViewController {
	...
	@IBAction func takePhoto(sender: AnyObject) {
		// I am replacing the standard buttons with my own,
		// so I have to forward the messages to the image picker.
	...
	private func prepareActivityView() {
		...
		let origin = CGPointMake(newSize.width  - (activitySize.width  / 2)
			,                    newSize.height - (activitySize.height / 2))
